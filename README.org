...
** Comments
   =pdf-continuous-scroll-mode= often gets confused when switching between
   buffers. It is annoying, but I am not sure if that would be easy to fix
   ...
   pressing =c c= (also remember when Emacs hangs, control can often be regained
   by pressing =C-g=).
